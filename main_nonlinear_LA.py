...
# Save dataset
if(chop):
   torch.save([train_input, train_target, train_init, cv_input_long, cv_target_long, test_input, test_target], DatafolderName+DatafileName) # type: ignore
else:
...
if(chop):
   KNet_Pipeline.NNTrain(sys_model,cv_input_long,cv_target_long,train_input,train_target,path_results,\
                         randomInit=True,train_init=train_init) # type: ignore
else:
   KNet_Pipeline.NNTrain(sys_model,cv_input_long,cv_target_long,train_input,train_target,path_results)
# Test Neural Network
